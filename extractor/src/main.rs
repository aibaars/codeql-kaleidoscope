...
fn main() -> std::io::Result<()> {
    ...
    let trap_compression = trap::Compression::from_env("CODEQL_KALEIDOSCOPE_TRAP_COMPRESSION");

    let file_list = matches.value_of("file-list").expect("missing --file-list");
    let file_list = fs::File::open(file_list)?;

    let language = tree_sitter_kaleidoscope::language();
    let schema = node_types::read_node_types_str("kaleidoscope", tree_sitter_kaleidoscope::NODE_TYPES)?;
