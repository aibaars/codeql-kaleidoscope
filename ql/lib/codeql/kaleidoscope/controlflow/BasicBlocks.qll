...
private import internal.ControlFlowGraphImplSpecific
private import internal.ControlFlowGraphImplShared
...
private module Cached {
  ...
  private predicate normalExitBB(BasicBlock bb) { bb.getANode() = TAnnotatedExitNode(_, true) }
...
private predicate entryBB(BasicBlock bb) { bb.getFirstNode() instanceof TEntryNode }
...
class AnnotatedExitBasicBlock extends BasicBlock {
  ...
  AnnotatedExitBasicBlock() { this.getANode() = TAnnotatedExitNode(_, normal) }
...
class ExitBasicBlock extends BasicBlock {
  ExitBasicBlock() { this.getLastNode() instanceof TExitNode }
}

private module JoinBlockPredecessors {
  private predicate id(ControlFlowElement x, ControlFlowElement y) { x = y }

  private predicate idOf(ControlFlowElement x, int y) = equivalenceRelation(id/2)(x, y)

  int getId(JoinBlockPredecessor jbp) {
    idOf(any(ControlFlowElement n | jbp.getFirstNode() = TElementNode(_, n, _)), result)
    or
    idOf(jbp.(EntryBasicBlock).getScope(), result)
  }

  string getSplitString(JoinBlockPredecessor jbp) {
    result = any(Splits s | jbp.getFirstNode() = TElementNode(_, _, s)).toString()
    or
    not exists(Splits s, string v | jbp.getFirstNode() = TElementNode(_, _, s) and v = s.toString()) and
    result = ""
  }
}
